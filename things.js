...
function thingProcess(type, settings, defaults) {
    ...
    // Sprite cycles
    var cycle;
    if (cycle = this.spriteCycle) {
        window.timeHandler.addSpriteCycle(this, cycle[0], cycle[1] || null, cycle[2] || null);
    }
    if (cycle = this.spriteCycleSynched) {
        window.timeHandler.addSpriteCycleSynched(this, cycle[0], cycle[1] || null, cycle[2] || null);
    }
}
...
function hitShellPlayer(one, two) {
    ...
    if (two.landing) {
        ...
        if (two.shelltoleft == shelltoleft) {
            ...
            window.timeHandler.addEvent(function (two) {
                --two.landing;
            }, 2, two);
        }
...
function movePirhanaNew(me, amount) {
    ...
    if (me.counter <= 0 || me.counter >= me.countermax) {
        me.movement = false;
        me.dir *= -1;
        window.timeHandler.addEvent(movePirhanaRestart, 35, me);
    }
}
...
function collideEnemy(one, two) {
    ...
    if (!map_settings.underwater && one.player && ((one.star && !two.nostar) || (!two.deadly && objectOnTop(one, two)))) {
        ...
        if (one.player && !one.star) {
            window.timeHandler.addEvent(function (one, two) {
                jumpEnemy(one, two);
            }, 0, one, two);
        } else {
            two.nocollide = true;
...
function movePodobooInit(me) {
    ...
    window.timeHandler.addEvent(podobooJump, me.betweentime, me);
    me.movement = false;
}
...
function throwHammer(me, count) {
    ...
    window.timeHandler.addEvent(function (me) {
        ...
        // ...and go again
        if (count > 0) {
            window.timeHandler.addEvent(throwHammer, 7, me, --count);
        } else {
            window.timeHandler.addEvent(throwHammer, 70, me, 7);
...
function moveCannonInit(me) {
    window.timeHandler.addEventInterval(
        function (me) {
...
function moveLakituInit(me) {
    ...
    window.timeHandler.addEventInterval(function (me) {
...
function coinEmerge(me, solid) {
    ...
    window.timeHandler.addEventInterval(coinEmergeMovement, 1, Infinity, me, solid);
    window.timeHandler.clearClassCycle(me, 0);
    addClass(me, "anim");
    window.timeHandler.addSpriteCycle(me, ["anim1", "anim2", "anim3", "anim4", "anim3", "anim2"], 0, 5);
}
...
function placePlayer(xloc, yloc) {
    ...
    if (map_settings.underwater) {
        ...
        window.timeHandler.addSpriteCycle(player, ["swim1", "swim2"], "swimming", 5);
    }
...
function playerGetsBig(me, noanim) {
    ...
    if (!noanim) {
        ...
        window.timeHandler.addSpriteCycle(me, stages, "shrooming", 6);
    }
...
function playerGetsSmall(me) {
    ...
    // Step two (t+21)
    window.timeHandler.addEvent(function (player) {
    ...
    window.timeHandler.addEvent(function (player) {
        ...
        window.timeHandler.addEvent(setThingSprite, 1, player);
    }, 42, player);
    // Step four (t+70);
    window.timeHandler.addEvent(function (player) {
...
function movePlayer(me) {
    ...
    if (Math.abs(me.xvel) < .14) {
        if (me.running) {
            ...
            addClass(me, "still");
            window.timeHandler.clearClassCycle(me, "running");
        }
    }
    // Not moving slowly
    else if (!me.running) {
    ...
    if (me.resting) {
        ...
        if (me.paddling) {
            ...
            window.timeHandler.clearClassCycle(me, "paddling");
            addClass(me, "running");
        }
    }
}
...
function playerStartRunningCycle(me) {
    ...
    me.running = window.timeHandler.addSpriteCycle(me, ["one", "two", "three", "two"], "running", setPlayerRunningCycler);
}
...
function playerPaddles(me) {
    if (!me.paddling) {
        ...
        window.timeHandler.clearClassCycle(me, "paddling_cycle");
        window.timeHandler.addSpriteCycle(me, ["paddle1", "paddle2", "paddle3", "paddle3", "paddle2", "paddle1", function () {
...
function unattachPlayer(me) {
    ...
    window.timeHandler.clearClassCycle(me, "climbing");
...
function killPlayer(me, big) {
    ...
    // Otherwise it's just a regular (enemy, time, etc.) kill
        ...
        // Otherwise, if this isn't a big one, animate a death
            ...
            // Make this look dead
            window.timeHandler.clearAllCycles(me);
...
function brickBump(me, character) {
    ...
    if (character.power > 1 && !me.contents) {
        ...
        return window.timeHandler.addEvent(brickBreak, 2, me, character);
    }
...
function placeShards(me) {
    for (let i = 0, shard; i < 4; ++i) {
        ...
        window.timeHandler.addEvent(killNormal, 350, shard);
    }
}
...
function blockBump(me, character) {
    ...
    switchClass(me, "unused", "used");
    checkContentsMushroom(me);
    window.timeHandler.addEvent(blockContentsEmerge, 7, me);
}
...
function vineEmerge(me, solid) {
    ...
    window.timeHandler.addEventInterval(vineStay, 1, 14, me, solid);
}
...
function touchVine(me, vine) {
    ...
    // setSize(me, 7, 8, true);
    me.climbing = window.timeHandler.addSpriteCycle(me, ["one", "two"], "climbing");
...
function makeCastleBlock(me, settings) {
    ...
    window.timeHandler.addEventInterval(castleBlockEvent, me.interval, Infinity, me);
}
...
function CastleFireBall(me, distance) {
    ...
    window.timeHandler.addSpriteCycle(me, ["one", "two", "three", "four"], 4);
}
...
function CastleAxeKillsBridge(bridge, axe) {
    ...
    if (bridge.width > 0) {
        window.timeHandler.addEvent(CastleAxeKillsBridge, 1, bridge, axe);
    } else {
        // Otherwise call the next step
        bridge.width = 0;
        window.timeHandler.addEvent(CastleAxeKillsBowser, 1, axe.bowser);
    }
...
function CastleAxeKillsBowser(bowser) {
    ...
    ++player.star;
    window.timeHandler.addEvent(CastleAxeContinues, 35, player);
}
...
function collideCastleNPC(me, collider) {
    killNormal(collider);
    me.keys.run = 0;
    window.timeHandler.addEvent(function (text) {
        var i;
        for (i = 0; i < text.length; ++i) {
            window.timeHandler.addEvent(proliferate, i * 70, text[i].element, {style: {visibility: "visible"}});
        }
        window.timeHandler.addEvent(endLevel, (i + 3) * 70);
    }, 21, collider.text);
}
...
function FlagCollisionTop(me, detector) {
    ...
    window.timeHandler.addSpriteCycle(me, ["one", "two"], "climbing");

    // Start moving the player down, as well as the end flag
    var endflag = window.mapsManager.getArea().getThingByID("endflag"),
